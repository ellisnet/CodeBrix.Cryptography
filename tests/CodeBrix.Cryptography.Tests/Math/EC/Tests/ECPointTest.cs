...
/**
 * Test class for {@link CodeBrix.Cryptography.Math.ec.ECPoint ECPoint}. All
...
public class ECPointTest
{
    ...
    public void TestLargeMInF2m()
    {
        ...
        {
            Assert.Equal("F2m m value out of range", e.Message);
        }
    }
        ...
        if (ECAlgorithms.IsF2mCurve(c))
        {
            ...
            if (null != h)
            {
                if (!h.TestBit(0))
                {
                    ...
                    Assert.True(order2.Twice().IsInfinity);
                    Assert.False(order2.IsValid());
                    ECPoint bad2 = g.Add(order2);
                    Assert.False(bad2.IsValid());
                    ECPoint good2 = bad2.Add(order2);
                    Assert.True(good2.IsValid());
                    ...
                    if (!h.TestBit(1))
                    {
                        ...
                        Assert.True(order4.Twice().Equals(order2));
                        Assert.False(order4.IsValid());
                        ECPoint bad4_1 = g.Add(order4);
                        Assert.False(bad4_1.IsValid());
                        ECPoint bad4_2 = bad4_1.Add(order4);
                        Assert.False(bad4_2.IsValid());
                        ECPoint bad4_3 = bad4_2.Add(order4);
                        Assert.False(bad4_3.IsValid());
                        ECPoint good4 = bad4_3.Add(order4);
                        Assert.True(good4.IsValid());
                    }
                }
            }
        }
    ...
    public void TestAddSubtractMultiplyTwiceEncoding()
    {
        ...
        foreach (string name in uniqNames)
        {
            ...
            {
                Assert.Equal(x9A.Curve.Field, x9B.Curve.Field);
                Assert.Equal(x9A.Curve.A.ToBigInteger(), x9B.Curve.A.ToBigInteger());
                Assert.Equal(x9A.Curve.B.ToBigInteger(), x9B.Curve.B.ToBigInteger());
                ...
                Assert.Equal(x9A.H, x9B.H);
                Assert.Equal(x9A.N, x9B.N);
