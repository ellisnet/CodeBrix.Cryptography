...
namespace CodeBrix.Cryptography.Math.EC.Rfc8032.Tests; //was previously: Org.BouncyCastle.Math.EC.Rfc8032.Tests;

public class Ed25519Test
{
    private readonly SecureRandom Random = new SecureRandom();

    // Upstream ran this from NUnit's [OneTimeSetUp]; a static constructor
    // gives the same once-per-type semantics under xUnit.
    static Ed25519Test()
    {
        Ed25519.Precompute();
    }
    ...
    public void TestEd25519Consistency()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
...

            {
                ...
                Assert.True(Arrays.AreEqual(pk, pk2), "Ed25519 consistent generation #" + i);
            }
            ...
            Assert.True(Arrays.AreEqual(sig1, sig2), "Ed25519 consistent signatures #" + i);

            {
                bool shouldVerify = Ed25519.Verify(sig1, 0, pk, 0, m, 0, mLen);

                Assert.True(shouldVerify, "Ed25519 consistent sign/verify #" + i);
            }
            {
...

            {
                ...
                Assert.False(shouldNotVerify, "Ed25519 consistent verification failure #" + i);
            }
            {
    ...
    public void TestEd25519ConsistencyExpandedKey()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
...

            {
                ...
                Assert.True(Arrays.AreEqual(pk, pk2), "Ed25519.ExpandedKey consistent generation #" + i);
            }
            ...
            Assert.True(Arrays.AreEqual(sig1, sig2), "Ed25519.ExpandedKey consistent signatures #" + i);

            {
                bool shouldVerify = Ed25519.Verify(sig1, 0, pk, 0, m, 0, mLen);

                Assert.True(shouldVerify, "Ed25519.ExpandedKey consistent sign/verify #" + i);
            }
            {
                bool shouldVerify = Ed25519.Verify(sig1, 0, publicPoint, m, 0, mLen);
...

            {
                ...
                Assert.False(shouldNotVerify, "Ed25519.ExpandedKey consistent verification failure #" + i);
            }
            {
                bool shouldNotVerify = Ed25519.Verify(sig1, 0, publicPoint, m, 0, mLen);
    ...
    public void TestEd25519CrossConsistencyExpandedKey()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
...

            {
                ...
                Assert.True(Arrays.AreEqual(pk, pk2), "Ed25519.ExpandedKey cross-consistent generation #" + i);
            }
            {
                var publicPoint = Ed25519.ExpandedKey.GeneratePublicKey(xk, 0);
...

            {
                ...
                Assert.True(Arrays.AreEqual(sig1, sig2), "Ed25519.ExpandedKey cross-consistent signatures #" + i);
            }
            {
                Ed25519.ExpandedKey.Sign(xk, 0, pk, 0, m, 0, mLen, sig2, 0);
    ...
    public void TestEd25519ctxConsistency()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            Assert.True(Arrays.AreEqual(sig1, sig2), "Ed25519ctx consistent signatures #" + i);

            {
                bool shouldVerify = Ed25519.Verify(sig1, 0, pk, 0, ctx, m, 0, mLen);

                Assert.True(shouldVerify, "Ed25519ctx consistent sign/verify #" + i);
            }
            {
...

            {
                ...
                Assert.False(shouldNotVerify, "Ed25519ctx consistent verification failure #" + i);
            }
            {
    ...
    public void TestEd25519phConsistency()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            Assert.True(Arrays.AreEqual(sig1, sig2), "Ed25519ph consistent signatures #" + i);

            {
                bool shouldVerify = Ed25519.VerifyPrehash(sig1, 0, pk, 0, ctx, ph, 0);

                Assert.True(shouldVerify, "Ed25519ph consistent sign/verify #" + i);
            }
            {
...

            {
                ...
                Assert.False(shouldNotVerify, "Ed25519ph consistent verification failure #" + i);
            }
            {
    ...
    public void TestPublicKeyValidationFull()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            Ed25519.GeneratePrivateKey(Random, sk);
            Ed25519.GeneratePublicKey(sk, 0, pk, 0);
            Assert.True(Ed25519.ValidatePublicKeyFull(pk, 0));
        }
    ...
    public void TestPublicKeyValidationPartial()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            Ed25519.GeneratePrivateKey(Random, sk);
            Ed25519.GeneratePublicKey(sk, 0, pk, 0);
            Assert.True(Ed25519.ValidatePublicKeyPartial(pk, 0));
        }
    ...
    public void TamingNonRepudiation()
    {
        ...
        Assert.False(Ed25519.Verify(sig, 0, pub, 0, msg1, 0, msg1.Length));
        Assert.False(Ed25519.Verify(sig, 0, pub, 0, msg2, 0, msg2.Length));
    }
    ...
    private static void InvalidPublicKeyFull(string sPK) =>
        Assert.False(Ed25519.ValidatePublicKeyFull(Hex.DecodeStrict(sPK), 0), sPK);

    private static void InvalidPublicKeyPartial(string sPK) =>
        Assert.False(Ed25519.ValidatePublicKeyPartial(Hex.DecodeStrict(sPK), 0), sPK);

    private static void ValidPublicKeyPartial(string sPK) =>
        Assert.True(Ed25519.ValidatePublicKeyPartial(Hex.DecodeStrict(sPK), 0), sPK);
    ...
    private static void CheckEd25519Vector(string sSK, string sPK, string sM, string sSig, string text)
    {
        ...
        Assert.True(Arrays.AreEqual(pk, pkGen), text);
