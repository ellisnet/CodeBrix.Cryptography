...
namespace CodeBrix.Cryptography.Cmp.Tests; //was previously: Org.BouncyCastle.Cmp.Tests;
...
public class ProtectedMessageTest
{
    ...
    public void TestProtectedMessage()
    {
        ...
        Assert.Equal(sender, message.Header.Sender);
        Assert.Equal(recipient, message.Header.Recipient);
    }
    ...
    public void TestSubsequentMessage()
    {
        ...
        Assert.Equal(ProofOfPossession.TYPE_KEY_ENCIPHERMENT, certReqMsg.Pop.Type);
    }
    ...
    public void TestServerSideKey()
    {
...

        {
            ...
            foreach (RecipientInformation recipientInfo in cmsEnvelopedData.GetRecipientInfos())
            {
                ...
                Assert.Equal(PkcsObjectIdentifiers.RsaEncryption, keyEncAlgOid);

                byte[] recData = recipientInfo.GetContent(kp.Private);
                Assert.Equal(privateKeyInfo, PrivateKeyInfo.GetInstance(recData));
            }
        }
    }
    ...
    private void ImplNotBeforeNotAfterTest(AsymmetricCipherKeyPair kp, DateTime? notBefore, DateTime? notAfter)
    {
        ...
        if (notBefore != null)
        {
            Assert.Equal(notBefore.Value, msg.GetCertTemplate().Validity.NotBefore.ToDateTime());
        }
        else
        {
        ...
        if (notAfter != null)
        {
            Assert.Equal(notAfter.Value, msg.GetCertTemplate().Validity.NotAfter.ToDateTime());
        }
        else
        {
