...
public class HqcVectorTest
{
    ...
    public void Consistency(HqcParameters parameters)
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            for (int j = 0; j < 10; ++j)
            {
                ...
                Assert.Equal(parameters.SessionKeySize, encapSecret.Length * 8);
                Assert.Equal(parameters.SecretLength, encapSecret.Length);
                Assert.Equal(parameters.EncapsulationLength, encapsulation.Length);
    ...
    public void ImplicitRejectionCoverage(HqcParameters parameters)
    {
        ...
        Assert.False(Arrays.AreEqual(tail1, tail2), parameters + ": rejection secret tail must depend on the ciphertext (FO implicit rejection)");
    }
