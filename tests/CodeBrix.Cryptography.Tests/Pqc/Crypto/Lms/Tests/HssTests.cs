...
public class HssTests
{
    ...
    public void TestGenerate()
    {
        ...
        //
        Assert.True(keyPair.GetPublicKey().Equals(HssPublicKeyParameters.GetInstance(Hex.Decode(expectedPk))), "HSSPrivateKeyParameterss equal are deserialization");
        ...
        //
        {
            ...
            Assert.True(Arrays.AreEqual(regenKeyPair.GetPublicKey().GetEncoded(), keyPair.GetPublicKey().GetEncoded()), "Both generated keys are the same");

            Assert.True(keyPair.GetKeys().Count == regenKeyPair.GetKeys().Count, "same private key size");
...

        {
            ...
            Assert.False(Arrays.AreEqual(differentKey.GetPublicKey().GetEncoded(), keyPair.GetPublicKey().GetEncoded()), "Both generated keys are not the same");
    ...
    public void TestVectorsFromReference_Expanded()
    {
        using (StreamReader sr = new StreamReader(SimpleTest.FindTestResource("pqc/crypto/lms/expansion.txt")))
        {
            ...
            Assert.Equal(1024, keyPair.GetUsagesRemaining());
            Assert.Equal(1024, keyPair.IndexLimit);
            Assert.Equal(0, keyPair.GetIndex());
            ...
            for (int i = 0; i < keyPair.IndexLimit; i++)
            {
                if (i == 500)
                {
                    ...
                    {
                        Assert.Equal("hss private key shard is exhausted", ex.Message);
                    }

                    pair = keyPair;
                    pubKeyGenerated = keyPair.GetPublicKey();

                    Assert.Equal(pubKeyGenerated, shard1.GetPublicKey());
                }
    ...
    public void TestSignUnitExhaustion()
    {
            ...
            while (ctr < 32769) // Just a number..
            {
                if (ctr % mod == 0)
                {
...

                    {
                        ...
                        {
                            Assert.Contains("nspk exceeded maxNspk", ex.Message);
                        }
        ...
        catch (Exception ex)
        {
            ...
            Assert.Contains("hss private key is exhausted", ex.Message);
        }
    }
