...
public class LmsTests
{
    ...
    public void TestCoefFunc()
    {
        ...
        Assert.Equal(0, LMOts.Coef(S, 7, 1));
        Assert.Equal(1, LMOts.Coef(S, 0, 4));
    }
    ...
    public void TestPrivateKeyRound()
    {
        ...
        try
        {
            ...
            Assert.Fail("Must fail as public key type not match signature type.");
        }
        catch (LmsException ex)
        {
            Assert.Contains("public key and signature ots types do not match", ex.Message);
        }
    }
