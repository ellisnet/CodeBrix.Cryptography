    ...
    : SimpleTest
{
    ...
    private void TestHash(TestDef pTest)
    {
        ...
        Assert.True(Arrays.AreEqual(myExpected, myBase), "baseDigest mismatch");

        /* Perform Xof hash */
        myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
        byte[] myOutput = new byte[pTest.theHash.Length];
        CheckMultiOutput(myDigest, myOutput);
        Assert.True(Arrays.AreEqual(pTest.theHash, myOutput), "digest mismatch");
    }
    ...
    private void TestKeyed(TestDef pTest)
    {
        ...
        Assert.True(Arrays.AreEqual(myExpected, myBase), "baseKeyedDigest mismatch");

        /* Perform Xof hash */
        myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
        byte[] myOutput = new byte[pTest.theKeyed.Length];
        CheckMultiOutput(myDigest, myOutput);
        Assert.True(Arrays.AreEqual(pTest.theKeyed, myOutput), "keyedDigest mismatch");
    }
    ...
    private void TestDerived(TestDef pTest)
    {
        ...
        Assert.True(Arrays.AreEqual(myExpected, myBase), "baseDerivedDigest mismatch");

        /* Perform Xof hash */
        myDigest.BlockUpdate(pTest.theData, 0, pTest.theLength);
        byte[] myOutput = new byte[pTest.theDerived.Length];
        CheckMultiOutput(myDigest, myOutput);
        Assert.True(Arrays.AreEqual(pTest.theDerived, myOutput), "derivedDigest mismatch");
    }
    ...
    private void CheckMultiOutput(Blake3Digest pDigest, byte[] pBuffer)
    {
        ...
        pDigest.Reset(mySave);
        Assert.True(Arrays.AreEqual(pBuffer, myAlt), "multiOutput mismatch");
    }
