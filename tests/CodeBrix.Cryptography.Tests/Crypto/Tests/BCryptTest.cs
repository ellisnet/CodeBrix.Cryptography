...
public class BCryptTest
{
    ...
    public void ShortKeys()
    {
        ...
        {
            Assert.Fail(string.Format("Hash for empty password should equal zeroed key", Hex.ToHexString(hashEmpty), Hex.ToHexString(hashZero1)));
        }
        ...
        {
            Assert.Fail(string.Format("Hash for empty password should equal zeroed key[4]", Hex.ToHexString(hashEmpty), Hex.ToHexString(hashZero4)));
        }
    ...
    private static void ImplTestVector(byte[] password, byte[] salt, int cost, byte[] expected)
    {
        ...
        {
            Assert.Fail(string.Format("Hash for " + Hex.ToHexString(password), Hex.ToHexString(expected), Hex.ToHexString(hash)));
        }
    }
    ...
    public void AddTerminator()
    {
        ...
        {
            Assert.Fail(string.Format("addTerminator=true should match pre-terminated test vector", Hex.ToHexString(expected), Hex.ToHexString(hashWithTerm)));
        }
        ...
        {
            Assert.Fail(string.Format("addTerminator=true must equal passwordToByteArray-fed generate", Hex.ToHexString(hashHelperFed), Hex.ToHexString(hashWithTerm)));
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            Assert.Fail(string.Format("addTerminator=false must equal legacy 3-arg generate", Hex.ToHexString(hashLegacy), Hex.ToHexString(hashWithoutTerm)));
        }
        ...
        {
            Assert.Fail(string.Format("addTerminator must be ignored when pwInput.length == 72", Hex.ToHexString(hash72Without), Hex.ToHexString(hash72With)));
        }
