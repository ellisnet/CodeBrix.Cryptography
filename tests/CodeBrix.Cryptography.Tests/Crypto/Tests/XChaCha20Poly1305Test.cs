...
public class XChaCha20Poly1305Test
{
    ...
    public void DraftVector()
    {
        ...
        Assert.Equal(expectedCipher.Length + expectedTag.Length, len);
        ...
        Assert.Equal(Hex.ToHexString(expectedCipher), Hex.ToHexString(actualCipher));
        Assert.Equal(Hex.ToHexString(expectedTag), Hex.ToHexString(actualTag));
        Assert.True(Arrays.AreEqual(expectedTag, enc.GetMac()), "XChaCha20-Poly1305 GetMac mismatch");
        ...
        Assert.Equal(plain.Length, decLen);
        Assert.True(Arrays.AreEqual(plain, decBytes), "XChaCha20-Poly1305 decryption mismatch");
    }
    ...
    public void RandomRoundTrip()
    {
        ...
        for (int i = 0; i < 50; ++i)
        {
            ...
            Assert.Equal(pLen, dl);
            Assert.True(Arrays.AreEqual(plain, pt), "round-trip: plaintext mismatch");
    ...
    public void ParameterUtilitiesGenerates24ByteIV()
    {
        ...
        Assert.Equal(24, ivParams.GetOctets().Length);
    }
    ...
    public void CipherUtilitiesProvidesRawStreamCipher()
    {
        ...
        Assert.True(Arrays.AreEqual(plain, streamPt), "CipherUtilities XCHACHA20 round-trip failed");
    }
    ...
    public void CipherUtilitiesProvidesAeadCipher()
    {
        ...
        Assert.Equal(Hex.ToHexString(expected), Hex.ToHexString(aeadCt));
    }
}
