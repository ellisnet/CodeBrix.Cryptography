...
public class IsaacTest
{
    ...
    private static void ImplTest(IsaacEngine engine, byte[] key, byte[] input, byte[] output)
    {
        ...
        Assert.True(Arrays.AreEqual(output, enc), "ciphertext mismatch");

        engine.Reset();
        engine.ProcessBytes(input, 0, input.Length, enc, 0);
        Assert.True(Arrays.AreEqual(output, enc), "ciphertext mismatch (reset)");

        engine.Init(false, new KeyParameter(key));
        engine.ProcessBytes(output, 0, output.Length, enc, 0);
        Assert.True(Arrays.AreEqual(input, enc), "plaintext mismatch");

        engine.Reset();
        engine.ProcessBytes(output, 0, output.Length, enc, 0);
        Assert.True(Arrays.AreEqual(input, enc), "plaintext mismatch (reset)");
    }
}
