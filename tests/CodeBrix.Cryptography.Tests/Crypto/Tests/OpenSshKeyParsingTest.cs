...
public class OpenSshKeyParsingTest
{
    ...
    public void TestDsa()
    {
        ...
        Assert.True(signer.VerifySignature(originalMessage, rs[0], rs[1]), "DSA test");
    }
    ...
    public void TestECDsa_CurvesFromSshKeyGen()
    {
        ...
        for (int i = 0; i != pairs.Length; i++)
        {
            ...
            signer.Init(false, pubSpec);
            Assert.True(signer.VerifySignature(originalMessage, rs[0], rs[1]), "ECDSA test post encoded / Decode");
        }
    ...
    private void DoECSigTest(AsymmetricKeyParameter pubSpec, AsymmetricKeyParameter privSpec)
    {
        ...
        signer.Init(false, pubSpec);
        Assert.True(signer.VerifySignature(originalMessage, rs[0], rs[1]), "ECDSA test");
    }
    ...
    public void TestEd25519()
    {
        ...
        signer.Init(false, pubSpec);
        signer.BlockUpdate(originalMessage, 0, originalMessage.Length);
        Assert.True(signer.VerifySignature(sig), "ED25519Signer test");
    }
    ...
    public void TestFailures()
    {
        ...
        {
            Assert.Equal("private key check values are not the same", iles.Message);
        }
        ...
        {
            Assert.Equal("encrypted keys not supported", iles.Message);
        }
    }
    ...
    private void DoRSATest(AsymmetricKeyParameter pubSpec, AsymmetricKeyParameter privSpec)
    {
        ...
        Assert.True(Arrays.AreEqual(originalMessage, result), "Result did not match original message");
    }
