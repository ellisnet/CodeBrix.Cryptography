...
public class Grain128AeadTest
{
    ...
    public void TestSplitUpdate()
    {
        ...
        {
            Assert.Contains("associated data must be added before plaintext/ciphertext", e.Message);
        }
...

    {
        ...
        {
            Assert.Contains("Grain-128AEAD Init parameters must include an IV", e.Message);
        }
        ...
        {
            Assert.Contains("Grain-128AEAD requires exactly 12 bytes of IV", e.Message);
        }
        ...
        {
            Assert.Contains("Grain-128AEAD key must be 128 bits long", e.Message);
        }
    }
