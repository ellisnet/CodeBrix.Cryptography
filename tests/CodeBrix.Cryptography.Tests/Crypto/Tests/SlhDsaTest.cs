...
using CodeBrix.Cryptography.Pqc.Crypto.Tests;
...
public class SlhDsaTest
{
    ...
    public static IEnumerable<object[]> ContextFastFiles_Data => ContextFastFiles.Select(v => new object[] { v });
    ...
    public static IEnumerable<object[]> ContextSlowFiles_Data => ContextSlowFiles.Select(v => new object[] { v });
    ...
    [MemberData(nameof(ContextFastFiles_Data))]
    ...
    [Theory(Skip = "Explicit test in the upstream NUnit suite; not run by default. Remove Skip to run it.")]
    [MemberData(nameof(ContextSlowFiles_Data))]
