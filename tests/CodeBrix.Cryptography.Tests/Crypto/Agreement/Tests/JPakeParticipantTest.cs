    ...
    : SimpleTest
{
    ...
    internal void TestSuccessfulExchange()
    {
        ...
        Assert.Equal(aliceKeyingMaterial, bobKeyingMaterial);
    }

    internal void TestIncorrectPassword()
    {
        JPakeParticipant alice = CreateAlice();
    ...
    internal void TestStateValidation()
    {
        JPakeParticipant alice = CreateAlice();
        JPakeParticipant bob = CreateBob();
        ...
        Assert.Equal(JPakeParticipant.STATE_INITIALIZED, alice.State);
        ...
        Assert.Equal(JPakeParticipant.STATE_ROUND_1_CREATED, alice.State);
        ...
        Assert.Equal(JPakeParticipant.STATE_ROUND_1_VALIDATED, alice.State);
        ...
        Assert.Equal(JPakeParticipant.STATE_ROUND_2_CREATED, alice.State);
        ...
        Assert.Equal(JPakeParticipant.STATE_ROUND_2_VALIDATED, alice.State);
        ...
        BigInteger aliceKeyingMaterial = alice.CalculateKeyingMaterial();
        Assert.Equal(JPakeParticipant.STATE_KEY_CALCULATED, alice.State);
        ...
        JPakeRound3Payload aliceRound3Payload = alice.CreateRound3PayloadToSend(aliceKeyingMaterial);
        Assert.Equal(JPakeParticipant.STATE_ROUND_3_CREATED, alice.State);
        ...
        JPakeRound3Payload bobRound3Payload = bob.CreateRound3PayloadToSend(bobKeyingMaterial);
        alice.ValidateRound3PayloadReceived(bobRound3Payload, aliceKeyingMaterial);
        Assert.Equal(JPakeParticipant.STATE_ROUND_3_VALIDATED, alice.State);
    ...
    internal void TestValidateRound1PayloadReceived()
    {
        // We're testing alice here. Bob is just used for help.
    ...
    internal void TestValidateRound2PayloadReceived()
    {
        // We're testing alice here. Bob is just used for help.
