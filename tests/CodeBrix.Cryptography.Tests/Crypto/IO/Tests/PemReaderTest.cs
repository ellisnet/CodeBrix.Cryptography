...
public class PemReaderTest
{
    ...
    public void TestMalformedInput()
    {
        ...
        using (var pemReader = new PemReader(new StringReader(raw)))
        {
            ...
            Assert.Equal("CERTIFICATE REQUEST", item.Type);
            Assert.Equal("CN=Test2SANs", subject);
        }
    }
    ...
    public void TestWithHeaders()
    {
        ...
        {
            Assert.Equal(items[0], item.Headers[t].Name);
            Assert.Equal(items[1], item.Headers[t].Value);
            t++;
        }
