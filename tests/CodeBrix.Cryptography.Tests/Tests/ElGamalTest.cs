...
public class ElGamalTest
{
	...
	private void DoTestGP(int size, int privateValueSize, BigInteger g, BigInteger p)
	{
		...
		Assert.Equal((size / 8) * 2, cipher.GetOutputSize(inBytes.Length));

		byte[] outBytes = cipher.DoFinal(inBytes);

		cipher.Init(false, keyPair.Private);

		Assert.Equal((size / 8) - 1, cipher.GetOutputSize(outBytes.Length));
		...
		Assert.True(spec.G.Equals(elParams.G) && spec.P.Equals(elParams.P), size + " bit public key encoding/decoding test failed on parameters");

		Assert.Equal(((ElGamalPublicKeyParameters)keyPair.Public).Y, pubKey.Y);
		...
		Assert.True(spec.G.Equals(elParams.G) && spec.P.Equals(elParams.P), size + " bit private key encoding/decoding test failed on parameters");

		Assert.Equal(((ElGamalPrivateKeyParameters)keyPair.Private).X, privKey.X);
	}
	...
	private void CheckKeySize(int privateValueSize, AsymmetricCipherKeyPair aKeyPair)
	{
		if (privateValueSize != 0)
		{
			...
			Assert.Equal(privateValueSize, key.X.BitLength);
		}
	}
