...
public class PkixNameConstraintsTest
{
    ...
    public void Basic()
    {
        ...
        {
            Assert.Equal("OtherName is from an excluded subtree.", e.Message);
        }
    ...
    public void TrailingDotBypass()
    {
        ...
        Assert.True(IsExcluded(EmailName("bank.com"), EmailName("ceo@bank.com.")), "trailing-dot email must be caught by the excluded bank.com subtree");

        // dNSName: exact and subdomain forms, including the dot-prefixed (proper-subtree) constraint.
        Assert.True(IsExcluded(DnsName("example.com"), DnsName("example.com.")), "exact host with a trailing dot must be caught");
        Assert.True(IsExcluded(DnsName("example.com"), DnsName("foo.example.com.")), "subdomain with a trailing dot must be caught");
        Assert.True(IsExcluded(DnsName(".example.com"), DnsName("foo.example.com.")), "subdomain with a trailing dot must be caught by a dot-prefixed constraint");
        Assert.False(IsExcluded(DnsName("example.com"), DnsName("notexample.com.")), "a sibling domain must not be caught");

        // uniformResourceIdentifier: the host trailing dot is stripped like the dNSName path.
        Assert.True(IsExcluded(UriName("competitor.example"), UriName("https://competitor.example./")), "trailing-dot URI host must be caught by the excluded competitor.example subtree");
    }
    ...
    public void DnsEmptyLabelRejected()
    {
        ...
        {
            Assert.Throws<PkixNameConstraintValidatorException>(() => new PkixNameConstraintValidator().IntersectPermittedSubtree(
                    new GeneralSubtree(DnsName(bad))));

            Assert.Throws<PkixNameConstraintValidatorException>(() => new PkixNameConstraintValidator().AddExcludedSubtree(new GeneralSubtree(DnsName(bad))));
        }
        ...
        Assert.True(IsExcluded(DnsName("example.com"), DnsName("foo..example.com")), "an empty label in a tested name must not escape the excluded subtree");
        Assert.False(IsPermitted(DnsName("example.com"), DnsName("foo..example.com")), "an empty label in a tested name must not be permitted");
    }
    ...
    public void DnsLeadingDotConstraintExcludesApex()
    {
        ...
        Assert.True(IsExcluded(DnsName(".example.com"), DnsName("foo.example.com")), "a subdomain must match the dot-prefixed constraint");
        Assert.False(IsExcluded(DnsName(".example.com"), DnsName("example.com")), "the apex must not match the dot-prefixed (proper-subtree) constraint");
        Assert.True(IsPermitted(DnsName(".example.com"), DnsName("a.b.example.com")), "a nested subdomain must be permitted by the dot-prefixed constraint");
        Assert.False(IsPermitted(DnsName(".example.com"), DnsName("example.com")), "the apex must not be permitted by the dot-prefixed (proper-subtree) constraint");

        // Set algebra across the two spellings: the plain form is the strictly broader set.
        Assert.True(IsExcludedAfterUnion(DnsName(".example.com"), DnsName("example.com"), DnsName("example.com")), "union with the plain form must keep the apex excluded");
        Assert.False(IsPermittedAfterIntersect(DnsName(".example.com"), DnsName("example.com"),
                DnsName("example.com")), "intersection must keep the narrower proper-subtree form: the apex is not permitted");
        Assert.True(IsPermittedAfterIntersect(DnsName(".example.com"), DnsName("example.com"),
                DnsName("foo.example.com")), "intersection must still permit subdomains");

        // Constraint-only: a tested dNSName with a leading dot fails closed.
        Assert.True(IsExcluded(DnsName("other.test"), DnsName(".example.com")), "a tested dNSName with a leading dot must fail closed");
    }
    ...
    public void EqualConstraintKeepsFirstRegisteredSpelling()
    {
        ...
        Assert.True(rendered.Contains("EXAMPLE.com") && !rendered.Contains("example.COM"), "an equal permitted email pair must keep the first-registered spelling");
        Assert.True(rendered.Contains("BANK.example") && !rendered.Contains("bank.EXAMPLE"), "an equal excluded dNSName pair must keep the first-registered spelling");
        Assert.True(rendered.Contains("HOST.example") && !rendered.Contains("host.EXAMPLE"), "an equal excluded URI pair must keep the first-registered spelling");
    }
    ...
    public void DirectoryNameEmptyBaseInert()
    {
        ...
        Assert.False(IsExcluded(emptyDn, DirectoryName("O=Org")), "an empty excluded base must match no name");
        ...
        {
            Assert.False(IsPermitted(emptyDn, DirectoryName("O=Org")), "an empty permitted base must match no name in SGP.22 mode");
        });

        // Excluded union, both registration orders: the empty base must not absorb the real subtree.
        Assert.True(IsExcludedAfterUnion(emptyDn, DirectoryName("O=Org"), DirectoryName("O=Org, CN=x")), "an empty excluded base must not absorb a later real subtree");
        Assert.True(IsExcludedAfterUnion(DirectoryName("O=Org"), emptyDn, DirectoryName("O=Org, CN=x")), "an empty excluded base must not absorb an earlier real subtree");

        // Permitted intersect: empty and real subtrees do not overlap, so the intersection is empty.
        Assert.False(IsPermittedAfterIntersect(emptyDn, DirectoryName("O=Org"), DirectoryName("O=Org, CN=x")), "an empty permitted base must not stand in for a real permitted subtree");
    }
    ...
    public void EmailUriEmptyLabelRejected()
    {
        ...
        foreach (string bad in malformed)
        {
            foreach (var name in new[] { EmailName(bad), UriName(bad) })
            {
                ...
                Assert.Throws<PkixNameConstraintValidatorException>(() => new PkixNameConstraintValidator().AddExcludedSubtree(new GeneralSubtree(name)));
            }
        }

        // Tested rfc822Name: the empty label historically escaped the excluded domain subtree.
        Assert.True(IsExcluded(EmailName(".example.com"), EmailName("user@foo..example.com")), "an empty label in a tested email host must not escape the excluded subtree");
        Assert.False(IsPermitted(EmailName(".example.com"), EmailName("user@foo..example.com")), "an empty label in a tested email host must not be permitted");

        // Tested URI: an extracted host with an empty label or a leading dot fails closed.
        Assert.True(IsExcluded(UriName("example.com"), UriName("https://foo..example.com/")), "an empty label in a tested URI host must not escape the excluded subtree");
        Assert.True(IsExcluded(UriName("example.com"), UriName("https://.example.com/")), "a leading dot in a tested URI host must fail closed");
        ...
        Assert.True(IsPermitted(EmailName("example.com"), EmailName("a..b@example.com")), "dots in the mailbox local part must not be rejected");
    }
    ...
    public void DirectoryNamePrefixBypass()
    {
        ...
        Assert.False(IsPermitted(permittedDN, prependedSubject), "subject with an RDN prepended before the permitted sequence must NOT be permitted");
    }
    ...
    public void UriHostExtractionBypass()
    {
        ...
        Assert.True(IsExcluded(UriName("2001:db8::1"), UriName("https://[2001:db8::1]:8443/x")), "bracketed IPv6 host with a port must be caught by the excluded 2001:db8::1 subtree");
        Assert.True(IsExcluded(UriName("2001:db8::1"), UriName("https://[2001:db8::1]/x")), "bracketed IPv6 host without a port must be caught");
        Assert.True(IsExcluded(UriName("2001:db8::1"), UriName("https://user:pw@[2001:db8::1]:8443/x")), "bracketed IPv6 host behind userinfo must be caught");
        ...
        Assert.True(IsExcluded(UriName("evil.example"),
                UriName("http://evil.example/x@allowed.example")), "'@' in the path must not be treated as userinfo");
        Assert.True(IsExcluded(UriName("competitor.example"),
                UriName("https://competitor.example?u=x@evil.example")), "'@' in the query must not be treated as userinfo");
        Assert.True(IsExcluded(UriName("competitor.example"),
                UriName("https://competitor.example#@evil.example")), "'@' in the fragment must not be treated as userinfo");

        // A genuine userinfo '@' before the host is still stripped.
        Assert.True(IsExcluded(UriName("host.example"), UriName("https://user@host.example/")), "userinfo before the host must be stripped");

        // Sanity: an unrelated host is not caught (extraction isn't over-matching).
        Assert.False(IsExcluded(UriName("competitor.example"), UriName("https://safe.example/")), "an unrelated URI host must not be caught");
    }
    ...
    public void IPv4MappedAddressBypass()
    {
        ...
        Assert.True(IsExcluded(IPName(ipv4Cidr24), IPName(IPv4Mapped(192, 0, 2, 5))), "IPv4-mapped SAN must be caught by the excluded IPv4 /24 constraint");

        // IPv4 SAN vs mapped constraint: caught (32-byte constraint normalizes to the /24).
        Assert.True(IsExcluded(IPName(mappedCidr24), IPName(Bytes(192, 0, 2, 5))), "IPv4 SAN must be caught by the excluded IPv4-mapped /24 constraint");

        // Out-of-range mapped SAN must NOT be caught (normalization isn't over-matching).
        Assert.False(IsExcluded(IPName(ipv4Cidr24), IPName(IPv4Mapped(198, 51, 100, 5))), "a mapped SAN outside the /24 must not be caught");
        ...
        Assert.False(IsExcluded(IPName(mappedNarrowMask), IPName(Bytes(192, 0, 2, 5))), "an IPv6-range constraint (mask < /96) must not be collapsed to match an IPv4 SAN");
    }
    ...
    public void SetAlgebraTrailingDotDns()
    {
        ...
        Assert.True(IsPermittedAfterIntersect(DnsName("example.com."), DnsName("example.com"),
                DnsName("foo.example.com")), "intersection of trailing-dot and plain forms must keep permitting subdomains");

        // The union of both forms must still catch a subdomain.
        Assert.True(IsExcludedAfterUnion(DnsName("example.com"), DnsName("example.com."),
                DnsName("foo.example.com")), "union of trailing-dot and plain forms must keep excluding subdomains");
    }
    ...
    {
        Assert.True(IsPermittedAfterIntersect(EmailName(".test.com."), EmailName(".test.com"),
                EmailName("user@abc.test.com")), "intersection of trailing-dot and plain domain-form email constraints must not be empty");
    }
    ...
    {
        Assert.True(IsExcludedAfterUnion(EmailName("test.com"), EmailName("test.com."),
                EmailName("user@test.com")), "union of trailing-dot and plain host-form email constraints must keep excluding");

        Assert.True(IsExcludedAfterUnion(UriName("test.de"), UriName("test.de."),
                UriName("http://test.de/abc")), "union of trailing-dot and plain URI host constraints must keep excluding");
    }
    ...
    public void UriEmptyNameEscapeUsesRawString()
    {
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => validator.CheckPermittedName(UriName("http://")));
    }
    ...
    public void LenientNonContiguousMappedMaskRoundsToCidr()
    {
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => strict.IntersectPermittedSubtree(new GeneralSubtree(IPName(mappedHoleFE))));
        ...
        Properties.WithThreadProperty(Properties.X509AllowLenientIPAddressMask, bool.TrueString, () =>
        {
            ...
            Assert.True(IsPermittedAfterIntersect(IPName(mappedHoleFE), IPName(mappedHole01),
                    IPName(Bytes(192, 0, 2, 5))), "salvaged mapped /120 masks must intersect to the IPv4 /24 and match an in-range SAN");
            Assert.False(IsPermittedAfterIntersect(IPName(mappedHoleFE), IPName(mappedHole01),
                    IPName(Bytes(198, 51, 100, 5))), "an IPv4 SAN outside the intersected range must not match");
        });
    }
    ...
    {
        Assert.True(IsExcluded(EmailName("TEST.com"), EmailName("user@test.com")), "an upper-cased host-form email constraint must still catch the lower-case mailbox");
        Assert.True(IsExcluded(DnsName("EXAMPLE.com"), DnsName("foo.example.com")), "an upper-cased dNSName constraint must still catch a lower-case subdomain");
        Assert.True(IsExcluded(UriName("HOST.example"), UriName("https://host.example/")), "an upper-cased URI constraint must still catch the lower-case host");
    }
    ...
    {
        Assert.True(IsPermittedAfterIntersect(EmailName("test.com."), EmailName("test.com"),
                EmailName("user@test.com")), "intersection of trailing-dot and plain host-form email constraints must not be empty");
    }
    ...
    {
        Assert.True(IsPermittedAfterIntersect(EmailName("u@test.com."), EmailName("u@test.com"),
                EmailName("u@test.com")), "intersection of trailing-dot and plain mailbox constraints must not be empty");
    }
    ...
    {
        Assert.True(IsPermittedAfterIntersect(UriName("test.de."), UriName("test.de"),
                UriName("http://test.de/abc")), "intersection of trailing-dot and plain URI host constraints must not be empty");
    }
    ...
    public void SetAlgebraHostNameDomainSubsumption()
    {
        ...
        Assert.False(IsPermittedAfterIntersect(EmailName(".example.com"), EmailName(".sub.example.com"),
                EmailName("user@x.example.com")), "email: intersection of nested domains keeps the narrower, so a name only in the broader fails");
        Assert.True(IsPermittedAfterIntersect(EmailName(".example.com"), EmailName(".sub.example.com"),
                EmailName("user@y.sub.example.com")), "email: a name within the narrower domain survives the intersection");
        Assert.True(IsExcludedAfterUnion(EmailName(".sub.example.com"), EmailName(".example.com"),
                EmailName("user@x.example.com")), "email: union of nested domains keeps the broader, so a name in it is excluded");

        // uniformResourceIdentifier shares the algebra (mirrored operand order), exercising the other branch.
        Assert.False(IsPermittedAfterIntersect(UriName(".example.com"), UriName(".sub.example.com"),
                UriName("http://x.example.com/")), "uri: intersection of nested domains keeps the narrower");
        Assert.True(IsExcludedAfterUnion(UriName(".sub.example.com"), UriName(".example.com"),
                UriName("http://x.example.com/")), "uri: union of nested domains keeps the broader");
        ...
        Assert.False(IsPermittedAfterIntersect(EmailName(".example.com"), EmailName(".example.org"),
                EmailName("user@x.example.com")), "email: intersection of disjoint domains is empty (name in first operand)");
        Assert.False(IsPermittedAfterIntersect(EmailName(".example.com"), EmailName(".example.org"),
                EmailName("user@x.example.org")), "email: intersection of disjoint domains is empty (name in second operand)");
    }
    ...
    {
        Assert.True(IsPermitted(EmailName("@abc.test.com"), EmailName("test@abc.test.com")), "an @host constraint must permit a mailbox on that exact host");
        Assert.True(IsExcluded(EmailName("@abc.test.com"), EmailName("test@abc.test.com")), "an @host constraint must exclude a mailbox on that exact host");
        Assert.False(IsExcluded(EmailName("@test.com"), EmailName("test@abc.test.com")), "an @host constraint must not match subdomain hosts");

        // RFC 1034 trailing-dot canonicalisation applies to the @host form too.
        Assert.True(IsExcluded(EmailName("@abc.test.com."), EmailName("test@abc.test.com")), "a trailing-dot @host constraint must still match");

        // Set algebra groups @host with the particular-address forms.
        Assert.True(IsPermittedAfterIntersect(EmailName("@test.com"), EmailName("@test.com"),
                EmailName("u@test.com")), "intersecting identical @host constraints must not empty the permitted set");
    }
    ...
    {
        Assert.False(IsExcluded(UriName("user@test.de"), UriName("http://user@test.de/x")), "a mailbox-shaped URI constraint must not match the host after its '@'");
        Assert.False(IsPermitted(UriName("user@test.de"), UriName("http://test.de/x")), "a mailbox-shaped URI constraint must not permit the host after its '@'");

        Assert.False(IsExcluded(UriName("@test.de"), UriName("http://user@test.de/x")), "an @host-shaped URI constraint must not match the host after its '@'");
        Assert.False(IsPermitted(UriName("@test.de"), UriName("http://test.de/x")), "an @host-shaped URI constraint must not permit the host after its '@'");
    }
    ...
    public void SetAlgebraMappedIpIntersection()
    {
        ...
        Assert.True(IsPermittedAfterIntersect(IPName(mappedCidr24), IPName(ipv4Cidr24), IPName(Bytes(192, 0, 2, 5))), "mapped and plain IPv4 forms of the same range must intersect");
        Assert.False(IsPermittedAfterIntersect(IPName(mappedCidr24), IPName(ipv4Cidr24),
                IPName(Bytes(198, 51, 100, 5))), "an IPv4 SAN outside the intersected range must not match");
    }
    ...
    public void MalformedIpConstraintRejected()
    {
        ...
        {
            PkixNameConstraintValidator validator = new PkixNameConstraintValidator();
            Assert.Throws<PkixNameConstraintValidatorException>(() => validator.AddExcludedSubtree(new GeneralSubtree(IPName(octets))));
            Assert.Throws<PkixNameConstraintValidatorException>(() => validator.IntersectPermittedSubtree(new GeneralSubtree(IPName(octets))));
        }
    }
    ...
    public void MalformedIpNameRejectedWhenConstrained()
    {
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => excluding.CheckExcludedName(IPName(malformed)));

        PkixNameConstraintValidator permitting = new PkixNameConstraintValidator();
        permitting.IntersectPermittedSubtree(new GeneralSubtree(IPName(ipv4Cidr24)));
        Assert.Throws<PkixNameConstraintValidatorException>(() => permitting.CheckPermittedName(IPName(malformed)));
    }
    ...
    public void EmptyIpNameNoLongerEscapes()
    {
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => validator.CheckPermittedName(IPName(Bytes())));
    }
    ...
    public void NonContiguousMaskRejectedByDefault()
    {
        ...
        PkixNameConstraintValidator validator = new PkixNameConstraintValidator();
        Assert.Throws<PkixNameConstraintValidatorException>(() => validator.IntersectPermittedSubtree(new GeneralSubtree(IPName(nonContiguous))));
        Assert.Throws<PkixNameConstraintValidatorException>(() => validator.AddExcludedSubtree(new GeneralSubtree(IPName(nonContiguous))));
    }
    ...
    public void LenientNonContiguousMaskRoundingDirection()
    {
        ...
        Properties.WithThreadProperty(Properties.X509AllowLenientIPAddressMask, bool.TrueString, () =>
        {
            ...
            Assert.True(IsPermitted(IPName(constraint), IPName(Bytes(192, 168, 1, 0))), "the network address is still permitted after narrowing to /32");
            Assert.False(IsPermitted(IPName(constraint), IPName(Bytes(192, 168, 1, 2))), "a permitted non-contiguous mask must narrow, rejecting a raw-mask match (under-permit)");

            // Excluded -> keep only the leading 1-bits => /24 (broader). 192.168.1.1 misses the raw mask
            // (1 & 5 == 1) but is excluded by the rounded /24.
            Assert.True(IsExcluded(IPName(constraint), IPName(Bytes(192, 168, 1, 1))), "an excluded non-contiguous mask must broaden, catching a raw-mask miss (over-exclude)");
        });
    }
    ...
    public void IpConstraintHostBitsZeroed()
    {
        ...
        Assert.True(IsExcluded(IPName(dirty), IPName(Bytes(192, 168, 1, 200))), "host bits in the base must not change which addresses match");
    }
    ...
    public void IpUnionDropsSubsumedRange()
    {
        ...
        Assert.Equal(justBroad, narrowFirst);
        ...
        Assert.Equal(justBroad, broadFirst);
        ...
        Assert.NotEqual(justBroad, disjoint);
    }
    ...
    public void CombinedChecks()
    {
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => validator.CheckName(DnsName("foo.example.com")));
        Assert.Throws<PkixNameConstraintValidatorException>(() => validator.CheckName(DnsName("other.com")));
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => dnValidator.CheckDN(new X509Name("O=Other, CN=x")));
        ...
        Assert.Throws<PkixNameConstraintValidatorException>(() => emailValidator.CheckEmail("ceo@bank.com"));
    }
    ...
    public void QuotedLocalPartEmailRejected()
    {
        ...
        Assert.True(IsExcluded(EmailName("evil.com"), EmailName("\"a@b\"@evil.com")), "an ambiguous multi-'@' rfc822Name must be caught (fail-closed) by an excluded constraint");
        ...
        Assert.True(IsExcluded(EmailName("excluded.example.com"),
                EmailName("\"user@evil.com\"@excluded.example.com")), "the reported quoted-local-part vector must be caught by the excluded subtree");

        // Any multi-'@' tested name fails closed under a permitted constraint too.
        Assert.False(IsPermitted(EmailName("bank.com"), EmailName("\"a@b\"@bank.com")), "an ambiguous multi-'@' rfc822Name must not satisfy a permitted constraint");
        ...
        Assert.False(IsExcluded(EmailName("evil.com"), EmailName("user@safe.com")), "a normal single-'@' address must not be affected");
        ...
        {
            Assert.False(IsExcluded(EmailName("evil.com"), EmailName("\"a@b\"@evil.com")), "the lenient valve must disable the ambiguity rejection");
        });
    }
    ...
    public void Sgp22NameConstraints()
    {
        ...
        Assert.False(IsPermitted(subtreeExtra, subjectExtra), "SGP.22 extra-attributes should be rejected by default");
        Assert.False(IsPermitted(subtreeSnFirst, subjectSnFirst), "SGP.22 leading serialNumber should be rejected by default");

        Properties.WithThreadProperty(Properties.X509Sgp22NameConstraints, bool.TrueString, () =>
        {
            // failure 1: subject carries extra attributes around the constrained O / serialNumber
            Assert.True(IsPermitted(subtreeExtra, subjectExtra), "SGP.22 extra-attributes should be permitted when enabled");

            // failure 2: serialNumber is the leading RDN and must match via startsWith
            Assert.True(IsPermitted(subtreeSnFirst, subjectSnFirst), "SGP.22 leading serialNumber should be permitted when enabled");

            // negative: a required organization that does not match is still rejected
            Assert.False(IsPermitted(subtreeExtra, DirectoryName("O=OTHER, serialNumber=89034011026140000000000000001332")), "mismatched organization must still be rejected");

            // negative: a serialNumber that is not a prefix is still rejected
            Assert.False(IsPermitted(subtreeExtra, DirectoryName("O=VALID, serialNumber=12340000000000000000000000000000")), "non-prefix serialNumber must still be rejected");

            // negative: a required attribute missing entirely is rejected
            Assert.False(IsPermitted(subtreeExtra, DirectoryName("C=ES, O=VALID, CN=VALID EUICC CD")), "missing required serialNumber must be rejected");
        });
    }
    ...
    public void Sgp22NonPrintableSerialNumber()
    {
        ...
        Properties.WithThreadProperty(Properties.X509Sgp22NameConstraints, bool.TrueString, () =>
        {
            Assert.False(IsPermitted(subtree, subjectUtf8Sn), "a non-PrintableString EID must not prefix-match the IIN (and must not throw)");
            Assert.True(IsPermitted(subtreeUtf8Sn, subjectUtf8Sn), "identical nonconforming serialNumbers must still match by ordinary equality");
            Assert.True(IsPermitted(subtreeUtf8Iin,
                    DirectoryName("O=VALID, serialNumber=89034011026140000000000000001332")), "a misencoded constraint IIN must still prefix-match a conforming EID");
        });
    }
    ...
    public void Sgp22IntersectFoldingStaysStrict()
    {
        ...
        Properties.WithThreadProperty(Properties.X509Sgp22NameConstraints, bool.TrueString, () =>
        {
            ...
            Assert.False(IsPermittedAfterIntersect(permuted1, permuted2, DirectoryName("O=Org, CN=x, CN=y")), "permuted-RDN permitted subtrees must intersect strictly (to nothing) in SGP.22 mode");
        });
    }
        ...
        string[] testIntersection)
    {
        ...
        for (int i = 0; i < testNames1.Length; i++)
        {
            ...
            Assert.Equal(constraintValidator, constraints2);
