...
namespace CodeBrix.Cryptography.Tests.Nist; //was previously: Org.BouncyCastle.Tests.Nist;
...
public class NistCertPathTest2
{
    ...
    {
        // PKITS 4.1.6 requires this path to be REJECTED. In the 2011 edition the
        // deliberate corruption leaves the signature BIT STRING non-octet-aligned
        // (padBits: 1), so the certificate is rejected while being decoded rather than
        // when its signature is checked. The rejection is what the test case is about;
        // only the exception it arrives as differs from the 2001 edition.
        ...
        Assert.Contains("octet-aligned", exception.Message);
    }
    ...
    {
        var expectedDate = FormatValidityDate(2011, 1, 1, 8, 30, 0);
        new PkitsTest()
    ...
    {
        var expectedDate = FormatRevocationDate(2010, 1, 1, 8, 30, 0);
        new PkitsTest()
    ...
    {
        var expectedDate = FormatRevocationDate(2010, 1, 1, 8, 30, 1);
        new PkitsTest()
    ...
    /// </remarks>
    [Fact(Skip = "CHECK -- \\\"Trust anchor for certification path not found.\\")]
    ...
    /// </remarks>
    [Fact(Skip = "CHECK getting \\\"Trust anchor for certification path not found.\\")]
    ...
    /// </remarks>
    [Fact(Skip = "CHECK -- Got: Trust anchor for certification path not found.")]
    ...
    /// </remarks>
    [Fact(Skip = "CHECK I think it is not using the new-signed-with-old")]
    ...
    /// certificate.
    /// </remarks>
    [Fact(Skip = "CHECK we may be too strict here, \\\"Intermediate certificate lacks BasicConstraints\\")]
    ...
    /// </remarks>
    [Fact(Skip = "CHECK not forming path, \\\"Trust anchor for certification path not found.\\")]
    ...
    {
        var expectedDate = FormatRevocationDate(2010, 6, 1, 8, 30, 0);
        new PkitsTest()
