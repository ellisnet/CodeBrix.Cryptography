...
public class TestEncodings
{
	...
	public void TestEC()
	{
		...
		Assert.True(tecPub.Equals(ecPub), "EC: public key to info back to public key");
		Assert.True(tecPriv.Equals(ecPriv), "EC: private key to info back to private key");
	}
    ...
    public void TestDH()
    {
        ...
        Assert.False(!testPrivKey.Equals(dhPrivate), "DH: Private key to info back to key");
        Assert.False(!testPubKey.Equals(dhPublic), "DH: Public key to info back to key");

        Assert.True(true, "Diffe Helman Test worked.");
    ...
    public void TestElGamal()
    {
            ...
            Assert.True(telPriv.Equals(elPriv), "ELGamal Private key to into back to private key.");
            Assert.True(telPub.Equals(elPub), "ELGamal Public key to into back to private key.");

            Assert.True(true, "ELGamal Test worked.");
    ...
    public void TestRsa()
    {
        ...
        Assert.False(!testResult.Equals(rsaPublic), "RSA: test failed on public key to info and back to public key.");
        ...
        Assert.False(!testResult.Equals(rsaPrivate), "RSA: private key to info back to private key.");

        Assert.True(true, "RSATest worked.");
    ...
    public void TestDSA()
    {
        ...
        Assert.False(!testResult.Equals(dsaPub), "DSA: test failed on public key to info and back to public key.");
        ...
        Assert.False(!testResult.Equals(dsaPriv), "DSA: private key to info back to private key.");

        Assert.True(true, "DSATest worked.");
    ...
    public void TestRsaPublicKeyInfoEncodingHasNullParameters()
    {
        ...
        Assert.True(hexEncoded.Contains(expectedSequence), "RSA AlgorithmIdentifier in SubjectPublicKeyInfo missing mandatory NULL parameters (05 00).");
    }
    ...
    public void TestSubjectPublicKeyInfoFactoryRsaConsistency()
    {
        ...
        Assert.Equal(PkcsObjectIdentifiers.RsaEncryption, algId.Algorithm);
        Assert.IsAssignableFrom<DerNull>(algId.Parameters);
    }
    ...
    public void TestGost2012()
    {
        ...
        AsymmetricKeyParameter akp = CodeBrix.Cryptography.Security.PrivateKeyFactory.CreateKey(keyInfo);
    }
}
