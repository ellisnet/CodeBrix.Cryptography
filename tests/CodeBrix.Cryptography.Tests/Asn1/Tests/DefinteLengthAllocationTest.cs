...
public class DefiniteLengthAllocationTest
{
    ...
    public void DeclaredLengthNotAllocatedUpFront()
    {
        ...
        Assert.Equal(declaredLength, octets.GetOctetsLength());
        int result = inStr.m_firstBulkReadLength;
        Assert.True(result >= 0 && result < declaredLength, $"first bulk read requested the full declared length ({result})");
    }
