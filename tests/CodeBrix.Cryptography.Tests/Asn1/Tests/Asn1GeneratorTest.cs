...
public class Asn1GeneratorTest
{
    ...
    private void TestTaggedBerSequence(int tagNo, bool declaredExplicit, BerSequence seq)
    {
        ...
        Assert.True(Arrays.AreEqual(expected, bOut.ToArray()), "BER seq [" + tagNo + "] explicit=" + declaredExplicit);
    ...
    private void TestTaggedDerSequence(int tagNo, bool declaredExplicit, DerSequence seq)
    {
        ...
        Assert.True(Arrays.AreEqual(expected, bOut.ToArray()), "DER seq [" + tagNo + "] explicit=" + declaredExplicit);
    ...
    private void CheckSequenceRoundTrip(byte[] encoding, int tagNo, bool declaredExplicit, Asn1Sequence expected)
    {
        ...
        Assert.True(taggedObject.HasContextTag(tagNo), "seq tagNo [" + tagNo + "]");
    ...
    private static void CheckBerOctetStringEncoding(byte[] encoding, byte[] expectedOctets, string message)
    {
        ...
        Assert.True(Arrays.AreEqual(expectedOctets, octetString.GetOctets()), message);
    }
        ...
        byte[] expectedOctets, string message)
    {
        ...
        Assert.True(taggedObject.HasContextTag(expectedTagNo), "octets tagNo [" + expectedTagNo + "]");

        var octetString = Asn1OctetString.GetInstance(taggedObject, declaredExplicit);
        Assert.True(Arrays.AreEqual(expectedOctets, octetString.GetOctets()), "octets content [" + expectedTagNo + "] explicit=" + declaredExplicit);
    }
}
