...
public class GeneralNameTest
{
    ...
    public void EdiPartyNameType()
    {
        ...
        Assert.True(Arrays.AreEqual(expected, both.GetEncoded(Asn1Encodable.Der)), "EDIPartyName encoding not as RFC 5280");

        EdiPartyName decoded = EdiPartyName.GetInstance(both.GetEncoded(Asn1Encodable.Der));
        Assert.Equal("assigner", decoded.NameAssigner.GetString());
        Assert.Equal("party", decoded.PartyName.GetString());
        ...
        Assert.True(Arrays.AreEqual(partyOnly, one.GetEncoded(Asn1Encodable.Der)), "optional nameAssigner encoding wrong");

        EdiPartyName decodedOne = EdiPartyName.GetInstance(one.GetEncoded(Asn1Encodable.Der));
        Assert.Null(decodedOne.NameAssigner);
        Assert.Equal("party", decodedOne.PartyName.GetString());
        ...
        Assert.Equal(GeneralName.EdiPartyName, gn.TagNo);
    ...
    private static void CheckIPAddressEncoding(string ipAddress, byte[] expectedEncoding, string message)
    {
        ...
        Assert.True(Arrays.AreEqual(expectedEncoding, nm.GetEncoded()), message);
    }
}
