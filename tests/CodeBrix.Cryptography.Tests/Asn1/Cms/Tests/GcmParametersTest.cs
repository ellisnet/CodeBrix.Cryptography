...
public class GcmParametersTest
{
    ...
    public void DefaultIcvLen() =>
        Assert.Equal(12, GcmParameters.GetInstance(SeqNoIcv()).IcvLen);
    ...
    public void ValidIcvLen()
    {
        ...
        {
            Assert.Equal(icvLen, GcmParameters.GetInstance(Seq(icvLen)).IcvLen);
            Assert.Equal(icvLen, new GcmParameters(new byte[12], icvLen).IcvLen);
        }
    }
}
