...
public class SignatureSubpacketsTest
{
    ...
    public void MinimalBodiesAccepted()
    {
        ...
        Assert.Equal(Features.FEATURE_SEIPD_V2, features.GetFeatures());
        ...
        Assert.Equal(2, trust.Depth);
        Assert.Equal(120, trust.TrustAmount);

        SignatureTarget target = new SignatureTarget(false, false, new byte[]{ 1, 8 });
        Assert.Equal(1, target.PublicKeyAlgorithm);
        Assert.Equal(8, target.HashAlgorithm);
        Assert.Empty(target.GetHashData());
        ...
        Assert.Equal(RevocationKeyTag.ClassDefault, revocationKey.SignatureClass);
        Assert.Equal(PublicKeyAlgorithmTag.RsaGeneral, revocationKey.Algorithm);
        Assert.Empty(revocationKey.GetFingerprint());

        RevocationReason revocationReason = new RevocationReason(false, false,
            new byte[]{ (byte)RevocationReasonTag.KeyRetired });
        Assert.Equal(RevocationReasonTag.KeyRetired, revocationReason.GetRevocationReason());
        Assert.Equal("", revocationReason.GetRevocationDescription());
        ...
        Assert.Equal("x", notationData.GetNotationName());
        Assert.Empty(notationData.GetNotationValueBytes());
    }
