...
public class ArmoredInputStreamCsfRejectPrefixedDashTest
{
    ...
    public void RejectsMalformedWhenConfigured()
    {
        ...
        {
            Assert.True(RejectMessage == e.Message, MessageMismatch);
        }
    }
    ...
    public void SurfacesMalformedBytesWhenLenient()
    {
        ...
        Assert.True(result.StartsWith("-Xpayload"), "Malformed payload MUST be returned unaltered");
    }
    ...
    public void LenientStreamStillDetectsArmorBoundary()
    {
        ...
        Assert.False(result.Contains("BEGIN PGP SIGNATURE"), "clear-text section must stop at the armor boundary, not consume the signature");
