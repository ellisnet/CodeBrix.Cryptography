...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private static void ValidateData(byte[] data, byte[] compressed)
    {
        ...
        Assert.True(Arrays.AreEqual(data, bytes), "compression test failed");
    }
    ...
    private static void CheckDecompressionLimit(byte[] data, CompressionAlgorithmTag type)
    {
        ...
        Assert.True(Arrays.AreEqual(data, Streams.ReadAll(c1.GetDataStream(data.Length))), "limit >= data should round-trip for type " + type);
        ...
        Assert.True(Arrays.AreEqual(data, Streams.ReadAll(c3.GetDataStream(-1))), "negative limit should be unbounded for type " + type);
    }
}
