...
//		private static readonly byte[] subKeyBindingCheckSum = Base64.Decode("3HU+");
    ...
    public void SubkeyExpiryFromForgedBinding()
    {
        ...
        Assert.Equal(GenuineSeconds, subKey.GetValidSeconds());
        Assert.Equal(GenuineSeconds, subKey.GetValidSeconds(masterKey));
        Assert.Equal(GenuineSeconds, pubRing.GetValidSeconds(subKey));
        ...
        Assert.False(forgedBinding.VerifyCertification(masterKey, subKey));

        // Legacy (unverified) path: the forged 0 wins - this is the reported vulnerability.
        Assert.Equal(0L, tamperedSubKey.GetValidSeconds());

        // Fixed (master-key-aware) path: the forged binding is ignored, genuine expiry preserved.
        Assert.Equal(GenuineSeconds, tamperedSubKey.GetValidSeconds(masterKey));
