...
public class NewTspTest
{
	...
	private void extensionTest(AsymmetricKeyParameter privateKey, X509Certificate cert, IStore<X509Certificate> certs)
    {
		...
		{
			Assert.True(e.Message == "request contains unknown policy");
		}
		...
		{
			Assert.True(e.Message == "request contains unknown extension");
		}
	...
	private void testNoNonse(AsymmetricKeyParameter privateKey, X509Certificate cert, IStore<X509Certificate> certs)
    {
		...
		Assert.True(new BigInteger("24").Equals(tstInfo.SerialNumber));

		Assert.True("1.2.3" ==  tstInfo.Policy);
		...
		Assert.True(0 == certificates.Count);
	}
	...
	private void testAccuracyWithCertsAndOrdering(AsymmetricKeyParameter privateKey, X509Certificate cert, IStore<X509Certificate> certs)
    {
		...
		Assert.True(1 == accuracy.Seconds);
		Assert.True(2 == accuracy.Millis);
		Assert.True(3 == accuracy.Micros);

		Assert.True(new BigInteger("23").Equals(tstInfo.SerialNumber));
		...
		Assert.True(2 == certificates.Count);
	...
	private void testAccuracyZeroCerts(AsymmetricKeyParameter privateKey, X509Certificate cert, IStore<X509Certificate> certs)
    {
		...
		Assert.True("1.2" == tstInfo.Policy);
	...
	private void overrideAttrsTest(AsymmetricKeyParameter privateKey, X509Certificate cert, IStore<X509Certificate> certs)
	{
		...
		Assert.True(cert.CertificateStructure.Issuer.Equals( sigCert.GetCerts()[0].IssuerSerial.Issuer.GetNames()[0].Name));
		Assert.True(cert.CertificateStructure.SerialNumber.Value.Equals( sigCert.GetCerts()[0].IssuerSerial.Serial.Value));
		Assert.True(Arrays.AreEqual(certHash, sigCert.GetCerts()[0].GetCertHash()));
		...
		Assert.True(cert.CertificateStructure.Issuer.Equals(sigCertV2.GetCerts()[0].IssuerSerial.Issuer.GetNames()[0].Name));
		Assert.True(cert.CertificateStructure.SerialNumber.Value.Equals(sigCertV2.GetCerts()[0].IssuerSerial.Serial.Value));
		Assert.True(Arrays.AreEqual(certHash256, sigCertV2.GetCerts()[0].GetCertHash()));
	...
	private void basicSha256Test(AsymmetricKeyParameter privateKey, X509Certificate cert, IStore<X509Certificate> certs)
	{
		...
		Assert.Equal((int)PkiStatus.Granted, tsResp.Status);
		...
		Assert.True(Arrays.AreEqual(certHash, sigCertV2.GetCerts()[0].GetCertHash()));
	}
		...
		IStore<X509Certificate> certs, Resolution resolution, string timeString)
	{
		...
		Assert.Equal(timeString, tsToken.TimeStampInfo.TstInfo.GenTime.TimeString);

		tsResp = tsRespGen.Generate(request, new BigInteger("23"), UnixEpoch.AddMilliseconds(9000));
		tsToken = tsResp.TimeStampToken;
		Assert.Equal("19700101000009Z", tsToken.TimeStampInfo.TstInfo.GenTime.TimeString);
		...
		if ((int)resolution > (int)Resolution.R_HUNDREDTHS_OF_SECONDS)
		{
			...
			tsToken = tsResp.TimeStampToken;
			Assert.Equal("19700101000009.99Z", tsToken.TimeStampInfo.TstInfo.GenTime.TimeString);
		}
		...
		if ((int)resolution > (int)Resolution.R_TENTHS_OF_SECONDS)
		{
			...
			tsToken = tsResp.TimeStampToken;
			Assert.Equal("19700101000009.9Z", tsToken.TimeStampInfo.TstInfo.GenTime.TimeString);
		}
	}
