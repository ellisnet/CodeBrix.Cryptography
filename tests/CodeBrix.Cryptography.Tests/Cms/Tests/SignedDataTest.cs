...
using CodeBrix.Cryptography.Asn1.Bsi;
using CodeBrix.Cryptography.Asn1.Cms;
using CodeBrix.Cryptography.Asn1.EdEC;
...
public class SignedDataTest
{
    ...
    public void AsVersion()
    {
        ...
        Assert.Equal(1, pinned.Version);

        // the content type, certificates and signer are untouched, and the signature still verifies.
        Assert.Equal(spcIndirectData, (pinned.SignedContent as CmsTypedData)?.ContentType);
        ...
        Assert.Equal(certsReplaced.Count, certsPinned.Count);
        ...
        Assert.Equal(1, new CmsSignedData(pinned.GetEncoded()).Version);
    }
    ...
    public void TestSha1WithRsaCounterSignature()
    {
        ...
        Assert.Equal(2, csSigners.Count);
    ...
    public void TestAddDigestAlgorithm()
    {
        ...
        Assert.Single(digestAlgorithms);
        Assert.Contains(sha1Null, digestAlgorithms);
        ...
        Assert.NotSame(oldS, s);

        var newDigestAlgorithms = new HashSet<AlgorithmIdentifier>(s.GetDigestAlgorithms());
        Assert.Equal(2, newDigestAlgorithms.Count);
        Assert.Contains(sha1Null, newDigestAlgorithms);
        Assert.Contains(ripeMD160, newDigestAlgorithms);
    }
        ...
        string signatureAlgorithm, DerObjectIdentifier sigAlgOid, AlgorithmIdentifier expectedDigAlgID)
    {
        ...
        var digestAlgorithms = new HashSet<AlgorithmIdentifier>(s.GetDigestAlgorithms());
        Assert.True(digestAlgorithms.Count > 0);

        if (expectedDigAlgID != null)
        {
            Assert.Contains(expectedDigAlgID, digestAlgorithms);
        }
        ...
        foreach (SignerInformation signer in signers.GetSigners())
        {
            ...
            if (sigAlgOid != null)
            {
                Assert.Equal(sigAlgOid, signer.SignatureAlgorithm.Algorithm);
                ...
                {
                    Assert.Equal(DerNull.Instance, signer.SignatureAlgorithm.Parameters);
                }
            }
        ...
        string digestAlgorithm)
    {
        ...
        foreach (SignerInformation signer in c)
        {
            ...
            Assert.Equal(digestAlgorithm, signer.DigestAlgorithmID.Algorithm.GetID());
    ...
    public void TestCertOrdering1()
    {
        ...
        Assert.Equal(OrigCert, a[0]);
        Assert.Equal(SignCert, a[1]);
        Assert.Equal(SignDsaCert, a[2]);
    }
    ...
    public void TestCertOrdering2()
    {
        ...
        Assert.Equal(3, a.Count);
        Assert.Equal(SignCert, a[0]);
        Assert.Equal(SignDsaCert, a[1]);
        Assert.Equal(OrigCert, a[2]);
    }
    ...
    public void TestSignerStoreReplacement()
    {
        ...
        Assert.Equal(NistObjectIdentifiers.IdSha224, signer.DigestAlgorithmID.Algorithm);
    ...
    public void TestSignerInfoGenCopyConstructor()
    {
        ...
        Assert.Same(newSignerInfoGen.Certificate, signerInfoGen.Certificate);
        Assert.Same(newSignerInfoGen.UnsignedAttributeTableGenerator, signerInfoGen.UnsignedAttributeTableGenerator);
        Assert.Same(newSignerInfoGen.SignedAttributeTableGenerator, signedAttrGen);
    }
    ...
    public void TestMalformedContentRejectedCleanly()
    {
        ...
        byte[] signedNoContent = new CodeBrix.Cryptography.Asn1.Cms.ContentInfo(
            CodeBrix.Cryptography.Asn1.Pkcs.PkcsObjectIdentifiers.SignedData, null).GetEncoded();
        Assert.Throws<CmsException>(() => new CmsSignedData(signedNoContent));

        byte[] envelopedNoContent = new CodeBrix.Cryptography.Asn1.Cms.ContentInfo(
            CodeBrix.Cryptography.Asn1.Pkcs.PkcsObjectIdentifiers.EnvelopedData, null).GetEncoded();
    ...
    public void TestOversizedDeclaredLengthRejected()
    {
        ...
        Assert.True(chain.Contains("out of bounds length found"),
