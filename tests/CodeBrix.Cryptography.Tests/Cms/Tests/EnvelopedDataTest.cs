...
using CodeBrix.Cryptography.Asn1.Ntt;
...
public class EnvelopedDataTest
{
    ...
    //    Assert.Equal(ed.EncryptionAlgOid, CmsEnvelopedGenerator.Aes128Cbc);
    ...
    //    {
    //        Assert.Equal(expectedLength, recipient.GetEncapsulation().Length);

    //        Assert.Equal(NistObjectIdentifiers.id_alg_ml_kem_512.GetID(), recipient.KeyEncryptionAlgOid);

    //        CmsTypedStream contentStream = recipient.GetContentStream(ReciMLKem512KP.Private);

    //        Assert.Equal(PkcsObjectIdentifiers.Data.GetID(), contentStream.ContentType);
    ...
    //    Assert.Equal(ed.EncryptionAlgOid, CmsEnvelopedGenerator.Aes256Cbc);
    ...
    //        Assert.Equal(NistObjectIdentifiers.id_alg_ml_kem_768.GetID(), recipient.KeyEncryptionAlgOid);
    ...
    //        Assert.Equal(NistObjectIdentifiers.id_alg_ml_kem_1024.GetID(), recipient.KeyEncryptionAlgOid);
	...
	public void TestKeyTrans()
	{
		...
		Assert.Equal(ed.EncryptionAlgOid, CmsEnvelopedGenerator.DesEde3Cbc);
    ...
    public void TestKeyTransRC2bit40()
    {
        ...
        Assert.Equal(ed.EncryptionAlgOid, CmsEnvelopedGenerator.RC2Cbc);

        RC2CbcParameter rc2P = RC2CbcParameter.GetInstance(ed.EncryptionAlgorithmID.Parameters);
        Assert.Equal(160, rc2P.RC2ParameterVersionData.IntValueExact);
	...
	public void TestKeyTransRC4()
	{
		...
		Assert.Equal(PkcsObjectIdentifiers.rc4, ed.EncryptionAlgorithmID.Algorithm);
	...
	public void TestKeyTransOdes()
	{
		...
		Assert.Equal(OiwObjectIdentifiers.DesCbc, ed.EncryptionAlgorithmID.Algorithm);
	...
	internal void TestKeyTransCamellia128()
	{
	...
	internal void TestKeyTransCamellia192()
	{
	...
	internal void TestKeyTransCamellia256()
	{
		...
		Type				asn1Params)
	{
		...
		Assert.Equal(checkOID, ed.EncryptionAlgorithmID.Algorithm);

		if (asn1Params != null)
		{
            Assert.True(asn1Params.IsInstanceOfType(ed.EncryptionAlgorithmID.Parameters.ToAsn1Object()));
		}
	...
	public void TestErroneousKek()
	{
		...
		foreach (RecipientInformation recipient in c)
		{
			Assert.Equal(recipient.KeyEncryptionAlgOid, NistObjectIdentifiers.IdAes128Wrap.Id);
	...
	private void TryKekAlgorithm(KeyParameter kek, DerObjectIdentifier algOid)
	{
		...
		foreach (RecipientInformation recipient in c)
		{
			Assert.Equal(algOid.Id, recipient.KeyEncryptionAlgOid);
		...
		byte[] expectedData, X509Certificate reciCert, AsymmetricKeyParameter reciPrivKey)
	{
		...
		Assert.True(Arrays.AreEqual(expectedData, actualData));
	}

	private static void ConfirmNumberRecipients(RecipientInformationStore recipients, int count)
	{
		Assert.Equal(count, recipients.GetRecipients().Count);
	}
	...
	public void TestPasswordRecipientIterationCountBound()
	{
		...
		{
			Assert.Throws<ArgumentException>(() => new Pkcs5Scheme2PbeKey("password".ToCharArray(), kdfAlg));
			Assert.Throws<ArgumentException>(() => new Pkcs5Scheme2Utf8PbeKey("password".ToCharArray(), kdfAlg));
		});
	}
	...
	public void TestRfc4134Ex5_1()
	{
		...
		Assert.Equal("1.2.840.113549.3.7", ed.EncryptionAlgOid);
	...
	public void TestRfc4134Ex5_2()
	{
		...
		Assert.Equal("1.2.840.113549.3.2", ed.EncryptionAlgOid);
    ...
    public void OriginatorInfo()
    {
        ...
        Assert.True(expectedSubject.Equivalent(originatorCerts[0].SubjectDN));

        Assert.Equal(CmsEnvelopedGenerator.DesEde3Cbc, env.EncryptionAlgOid);
    }
    ...
    //    Assert.Equal(ed.EncryptionAlgOid, CryptoProObjectIdentifiers.GostR28147Gcfb.Id);
    ...
    //    foreach (RecipientInformation recipient in c)
    //    {
    //        Assert.Equal(recipient.KeyEncryptionAlgOid, //            RosstandartObjectIdentifiers.id_tc26_agreement_gost_3410_12_256.Id);

    //        byte[] recData = recipient.GetContent(privKey);

    //        Assert.Equal("Hello World!", Strings.FromByteArray(recData));
    //    }
    ...
    //    //}
    //    //Assert.True(collection[0] is RecipientInformation);
    //}
    ...
    public void TestKeyTransRsaPkcs1PaddingOracleClosed()
    {
        ...
        Assert.True(Arrays.AreEqual(data, DecryptFirst(ed, ReciKP_2048.Private)));
        ...
        var rsaEncrypt = new CodeBrix.Cryptography.Crypto.Encodings.Pkcs1Encoding(
            new CodeBrix.Cryptography.Crypto.Engines.RsaEngine());
        ...
        Assert.NotEqual(BadPaddingMessage, TryDecryptExpectFailure(badPadding, ReciKP_2048.Private));
        Assert.NotEqual(BadPaddingMessage, TryDecryptExpectFailure(validPaddingWrongKey, ReciKP_2048.Private));
    }
    ...
    public void TestKeyTransRsaPkcs1UnknownContentKeySize()
    {
        ...
        Assert.NotEqual(BadPaddingMessage, strict);
        Assert.Contains("no fixed size", strict);
        ...
        Assert.Equal(BadPaddingMessage, lenient);
    }
    ...
    {
        var raw = new CodeBrix.Cryptography.Crypto.Engines.RsaEngine();
    ...
    {
        CodeBrix.Cryptography.Asn1.Cms.ContentInfo contentInfo = ed.ContentInfo;
        var envelopedData = CodeBrix.Cryptography.Asn1.Cms.EnvelopedData.GetInstance(contentInfo.Content);

        var keyTrans = (CodeBrix.Cryptography.Asn1.Cms.KeyTransRecipientInfo)
            CodeBrix.Cryptography.Asn1.Cms.RecipientInfo.GetInstance(envelopedData.RecipientInfos[0]).Info;

        var newKeyTrans = new CodeBrix.Cryptography.Asn1.Cms.KeyTransRecipientInfo(
        ...
        {
            encryptedContentInfo = new CodeBrix.Cryptography.Asn1.Cms.EncryptedContentInfo(
                encryptedContentInfo.ContentType, contentEncryptionAlgorithm,
                encryptedContentInfo.EncryptedContent);
        }

        var newEnvelopedData = new CodeBrix.Cryptography.Asn1.Cms.EnvelopedData(
            envelopedData.OriginatorInfo,
            new DerSet(new CodeBrix.Cryptography.Asn1.Cms.RecipientInfo(newKeyTrans)),
            encryptedContentInfo,
            envelopedData.UnprotectedAttrs);

        return new CmsEnvelopedData(
            new CodeBrix.Cryptography.Asn1.Cms.ContentInfo(contentInfo.ContentType, newEnvelopedData));
    }
	...
	private void VerifyECKeyAgreeVectors(AsymmetricKeyParameter privKey, string wrapAlg, byte[] message)
	{
		...
		Assert.Equal(wrapAlg, ed.EncryptionAlgOid);
		...
		foreach (RecipientInformation recipient in c)
		{
			Assert.Equal("1.3.133.16.840.63.0.2", recipient.KeyEncryptionAlgOid);
	...
	private void VerifyECMqvKeyAgreeVectors(AsymmetricKeyParameter privKey, string wrapAlg, byte[] message)
	{
		...
		foreach (RecipientInformation recipient in c)
		{
			Assert.Equal("1.3.133.16.840.63.0.16", recipient.KeyEncryptionAlgOid);
