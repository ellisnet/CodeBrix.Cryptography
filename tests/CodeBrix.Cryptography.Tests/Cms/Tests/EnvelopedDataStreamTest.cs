...
public class EnvelopedDataStreamTest
{
	...
	public void TestWorkingData()
	{
		...
		Assert.Equal(ep.EncryptionAlgOid, CmsEnvelopedGenerator.Aes128Cbc);
...
//            Key					priKey = keyFact.generatePrivate(keySpec);
		...
		foreach (RecipientInformation recipient in c)
		{
			...
			Assert.True(Arrays.AreEqual(data, compare));
		}
	}
		...
		byte[]	expectedData)
	{
		...
		Assert.Equal(ep.EncryptionAlgOid, expectedOid);
		...
		foreach (RecipientInformation recipient in c)
		{
			...
			Assert.True(Arrays.AreEqual(expectedData, CmsTestUtil.StreamToByteArray(
	...
	public void TestKeyTransAes128BufferedStream()
	{
		...
		Assert.Equal(unbufferedLength, bOut.ToArray().Length);
	}
	...
	public void TestKeyTransAes128Buffered()
	{
		...
		Assert.True(unbufferedLength < bOut.ToArray().Length);
	}
	...
	public void TestKeyTransAes128Throughput()
	{
		...
		if (e.MoveNext())
		{
			...
			Assert.True(Arrays.AreEqual(data, dataOut.ToArray()));
		}
		else
		{
	...
	public void TestKeyTransAes128()
	{
		...
		using (var ep = new CmsEnvelopedDataParser(bOut.ToArray()))
		{
            ...
            foreach (RecipientInformation recipient in recipients.GetRecipients())
            {
                ...
                Assert.True(Arrays.AreEqual(data, CmsTestUtil.StreamToByteArray(recData.ContentStream)));
            }
        }
    }
	...
	public void TestAesKek()
	{
		...
		using (var ep = new CmsEnvelopedDataParser(bOut.ToArray()))
		{
            ...
            Assert.Equal(ep.EncryptionAlgOid, CmsEnvelopedGenerator.DesEde3Cbc);

            foreach (RecipientInformation recipient in recipients.GetRecipients())
            {
                Assert.Equal("2.16.840.1.101.3.4.1.25", recipient.KeyEncryptionAlgOid);
