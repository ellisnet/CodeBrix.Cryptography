...
public class TlsTestSuite
{
    ...
    public static IEnumerable<object[]> Suite()
    {
        var testSuite = new List<object[]>();
        AddAllTests(testSuite, TlsTestConfig.CRYPTO_BC, TlsTestConfig.CRYPTO_BC);
        return testSuite;
    }

    private static void AddAllTests(IList<object[]> testSuite, int clientCrypto, int serverCrypto)
    {
    ...
    private static void AddFallbackTests(IList<object[]> testSuite, int clientCrypto, int serverCrypto)
    {
    ...
    private static void AddVersionTests(IList<object[]> testSuite, ProtocolVersion version, int clientCrypto,
        int serverCrypto)
    {
        string prefix = GetCryptoName(clientCrypto) + "_" + GetCryptoName(serverCrypto) + "_"
    ...
    private static void AddTestCase(IList<object[]> testSuite, TlsTestConfig config, string name)
    {
        testSuite.Add(new object[] { config, name });
    }
