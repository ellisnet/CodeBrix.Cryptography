...
using CodeBrix.Cryptography.Tests;
...
namespace CodeBrix.Cryptography.Pkcs.Tests; //was previously: Org.BouncyCastle.Pkcs.Tests;
    ...
    : SimpleTest
{
 ...
 + "LOhDDVV1zCxAAQ7XjkwCAgfQ");
    ...
    public void CertificateAliasConsistency()
    {
        ...
        for (int i = 0; i != certs.Length; i++)
        {
            Assert.Equal("cert-" + i, store.GetCertificateAlias(certs[i]));
        }

        Assert.Null(store.GetCertificateAlias(absent));
        ...
        for (int i = 0; i != certs.Length; i++)
        {
            Assert.Equal("cert-" + i, inStore.GetCertificateAlias(certs[i]));
        }

        Assert.Null(inStore.GetCertificateAlias(absent));
    }
    ...
    public void RawKeyBagNoAttributes()
    {
        ...
        Assert.NotNull(storeA.GetKey(alias));
        ...
        Assert.NotNull(storeB.GetKey("rawKeyBag"));
    }
    ...
    public void RawKeyBagStore()
    {
        ...
        Assert.True(store.IsKeyEntry("ONVIF_Test_Alias"));
    }
