	...
	: IStreamCipher
{
    ...
    internal static void SalsaCore(int rounds, ReadOnlySpan<uint> input, Span<uint> output)
    {
        ...
        if (CodeBrix.Cryptography.Runtime.Intrinsics.X86.Sse41.IsEnabled &&
    ...
    // The Sse2 shift intrinsics declare their count parameter [ConstantExpected], which
    // makes CA1857 fire on this shared helper because the rotation amount arrives as a
    // parameter. Specialising the helper per rotation amount to satisfy the analyzer
    // would rewrite hand-tuned intrinsics code for no functional gain, so the rule is
    // disabled for this method only.
