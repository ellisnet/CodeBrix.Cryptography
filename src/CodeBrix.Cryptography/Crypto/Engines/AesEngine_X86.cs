...
public struct AesEngine_X86
    : IBlockCipher
{
    public static bool IsSupported => CodeBrix.Cryptography.Runtime.Intrinsics.X86.Aes.IsEnabled;

    // Aes.KeygenAssist declares its round-constant parameter [ConstantExpected], which
    // makes CA1857 fire on the calls below that pass a loop-carried round constant. The
    // key schedule is generated in a loop by design; unrolling it to satisfy the
    // analyzer would rewrite hand-tuned intrinsics code for no functional gain, so the
    // rule is disabled for this method only.
...
#pragma warning restore CA1857
    ...
    {
        if (CodeBrix.Cryptography.Runtime.Intrinsics.Vector.IsPackedLittleEndian)
